...
def main() -> None:
    ...
    if use_policy:
        ...
        net = load_policy(os.environ.get("PN_MODEL_PATH", os.path.join(os.path.dirname(HERE), "models", "policy.npz")))
    ...
    for fen in POSITIONS:
        ...
        print(
            ...
            f"q {st.qnodes:7d} ({100*st.qnodes/max(1,st.nodes):.0f}%) tt {st.tt_hits:6d} pol {st.policy_calls:5d} "
            f"cpu-nps {int(st.nodes/max(cpu,1e-6)):6d} cpu {cpu:.2f}s wall {wall:.2f}s "
            f"pv {' '.join(m.uci() for m in r.pv[:5])}"
        )
    print(f"TOTAL nodes {total_nodes} cpu {total_cpu:.2f}s cpu-nps {int(total_nodes/max(total_cpu,1e-6))}")
